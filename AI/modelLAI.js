...
function getHistory(sym, periodo, stl) {
  ...
  ws.onmessage = function (msg) {
    ...
    if (stl === 'ticks') {
      ...
      console.log('Falha na aquisição dos dados!')
    }
  };
}
...
async function validate(X, Y, epochs, model, real, sma, max, min) {
  ...
  tfvis.render.linechart({
    ...
    values: [real, sma, train_values, valid_values],
    series: ['Original', 'SMA', 'Train Data', 'Validate Data']
  }, {
    zoomToFit: true,
    xLabel: 'Time',
    yLabel: 'Close',
...
async function main(X, Y, epochs, real, sma) {
  ...
  await result.model.save(tf.io.browserHTTPRequest(
    'http://yurialmeida.com/AI/json', {
      method: 'PUT'
      // headers: {
      //   mode: 'no-cors'
      // }
